# -*- coding: utf-8 -*-
"""Projek MLT Raka.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1SJXXNje0YDto5oukKF0qf3VKlDEUZDwd
...
Berdasarkan barplot perbandingan performa model di atas yang mencakup metrik Akurasi, F1-Score, Presisi, dan Recall, dapat ditarik beberapa *insight* mengenai kinerja masing-masing model:

* **SVM (Optuna) dan Logistic Regression (Optuna) menunjukkan performa yang sangat konsisten dan unggul** di hampir semua metrik.
    * **Akurasi:** SVM (Optuna) memimpin dengan 98.33%, diikuti sangat dekat oleh Logistic Regression (Optuna) dengan 97.92%.
    * **F1-Score:** Sama halnya, SVM (Optuna) mencapai 98.32%, sementara Logistic Regression (Optuna) sedikit di bawahnya dengan 97.91%.
    * **Presisi:** SVM (Optuna) dan Logistic Regression (Optuna) juga mendominasi dengan 98.55% dan 98.17% secara berturut-turut.
    * **Recall:** Pola yang serupa terlihat dengan SVM (Optuna) pada 98.33% dan Logistic Regression (Optuna) pada 97.92%.

* **Multinomial Naive Bayes menunjukkan performa yang baik, namun sedikit di bawah kedua model teratas.**
    * Model ini mencapai Akurasi 92.08% dan F1-Score 91.67%, yang merupakan angka yang solid.
    * Presisi 93.48% menunjukkan kemampuan yang cukup baik dalam menghindari *false positives*.
    * Recall 92.08% mengindikasikan kemampuan yang baik dalam menemukan semua *true positives*.

* **XGBoost (Optuna) berada di posisi terakhir di antara keempat model.**
    * Meskipun telah dioptimasi dengan Optuna, XGBoost menunjukkan Akurasi 90.83% dan F1-Score 90.75%, yang paling rendah dibandingkan model lainnya.
...
Berdasarkan perbandingan metrik Akurasi, F1-Score, Presisi, dan Recall, **model SVM (Optuna) dan Logistic Regression (Optuna) merupakan model terbaik** karena secara konsisten menunjukkan skor tertinggi di semua metrik evaluasi. Perbedaan performa antara keduanya sangat tipis, dengan SVM (Optuna) sedikit unggul dalam sebagian besar metrik.
